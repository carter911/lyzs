...
class Cron extends Controller
{
	...
	public function get_project()
	{
		...
		try {
			$id = Db::name('project')->order('sgb_id desc')->field('sgb_id')->find();
			...
			$curl = curl_init();
			//设置抓取的url
			curl_setopt($curl, CURLOPT_URL, 'http://api.e-shigong.com/serverProjectList?companyId=44&start=' . $id);
			//设置头文件的信息作为数据流输出
	...
	public function update_project()
	{
		...
		$time = date("Y-m-d 00:00:00", time());
		try {
			$id = Db::name('project')->where('update_time','lt',$time)->order('update_time asc')->field('sgb_id,id')->find();
			echo Db::name('project')->getLastSql().'</br>';
			echo '更新项目详情'.var_export($id,true).'<br/>';
			$id = empty($id) ? 0 : $id['sgb_id'];
			echo '开始id' . $id . '</br>';
	...
	public function update_image()
	{
		...
		try {
			...
			$curl = curl_init();
			//设置抓取的url
			curl_setopt($curl, CURLOPT_URL, 'http://api.e-shigong.com/serverUpdate?companyId=44');
			//设置头文件的信息作为数据流输出
			curl_setopt($curl, CURLOPT_HEADER, 1);
			//设置获取的信息以文件流的形式返回，而不是直接输出。
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
			...
			foreach ($data['data'] as $key => $val) {
				...
				Db::name('project')->wehre(['sgb_id' => $val['id']])->save($arr);
			}
